...
def beautify(defaultpath, df,iselltype,rowlim,htlname,pth,glossary,ftr,pgdf,finaladop,rateshopfile,hcap2,rng,yday):
    logging.debug('------------------------------------------------------------')
...
def isellbeautify(defaultpath, df, htlname, pth2, name_win2, isellrange, glossary, ftr, pgdf, finaladop, acc_man, rateshopfile, hCap,rng,yday):
    ...
    beautify(defaultpath, df,'internal',isellrange+rng+6,htlname,pth,glossary,ftr,pgdf,finaladop,rateshopfile,hcap2,rng,yday)
    #--------------------------------------------------------------------------
    ...
    logging.debug('iSell Range (isellrange) : {}'.format(isellrange+rng))
    ...
    if str(int(name_win2)) != '0':        
        ...
        beautify(defaultpath, df,'client',isellrange+6+rng,htlname,pth,glossary,ftr,pgdf,finaladop,rateshopfile,hcap2,rng,yday)
        ...
        if dow in ['Mon','Fri']:
            ...
            beautify(defaultpath,df,'client',isellrange+rng+6,htlname,pth,glossary,ftr,pgdf,finaladop,rateshopfile,hcap2,rng,yday)
            logging.info("Client iSell Dumped !!!")
            ...
            beautify(defaultpath,df,'client',36,htlname,pth,glossary,ftr,pgdf,finaladop,rateshopfile,hcap2,rng,yday)
            logging.info("Client iSell Dumped !!!")
