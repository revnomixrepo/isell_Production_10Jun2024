...
def TBhnfconv(df_hnf,maxcap,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    ...
    df_hnf2=frame(df_hnf,isellrange,rng)
...
def UKhnfconv(df_hnf,maxcap,isellrange,rng):
    logging.debug('------------------------------------------------------------')
...
def hnfconv(hnf,totalcap,isellrange,rng):
...
def occframe(df1,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    logging.debug('Module:iSell_fun_02, SubModule:occframe') 
       
    
    ddmmyy1 = datetime.now()
    ddmmyy = ddmmyy1 - timedelta(days=1)    
...
def frame(df1,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    logging.debug('Module:iSell_fun_02, SubModule:frame') 
    
    df1 = pd.DataFrame(df1)
    ddmmyy1 = datetime.now()-timedelta(days=1)
    tday = ddmmyy1.strftime("%d-%b-%Y")
    index=pd.date_range(tday,periods= isellrange+rng)
    ...
    return(merged)
def frameMax(df1, isellrange,rng):
    df1 = pd.DataFrame(df1)
    ddmmyy = datetime.now()-timedelta(days=1)
...
def RateShop(rsfile,isellrange,rng,htl,cc_value):
    logging.debug('------------------------------------------------------------')
    ...
    lrate4= frame(lrate3,isellrange,rng)
