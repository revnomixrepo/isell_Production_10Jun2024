...
def TravelClick(rfile, ifile, cmdata2, ftr, msrate, rateplan, isellrange,rng):
    logging.debug('------------------------------------------------------------')
    ...
    rt_df = iSell_fun_02.frame(rt_df, isellrange,rng)
    inv_df = iSell_fun_02.frame(inv_df, isellrange,rng)
    flt_df = iSell_fun_02.frame(flt_df, isellrange,rng)
...
def BookingHotel_CM(pcdata, cmdata, ftr, msrate, isellrange,rng):
    logging.debug('------------------------------------------------------------')
    ...
    availframe = iSell_fun_02.frame(cmfin3, isellrange,rng)
...
### READ RATE FILE
    ...
    msrateframe = iSell_fun_02.frame(df2, isellrange,rng)
...
def CM_RezNext(cmdf,msrate,ftr,chnnel,isellrange,rng):
    logging.debug('------------------------------------------------------------')
...
def CM_ResAvenue(cmdata,pcdata,ftr,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    ...
    df5 = iSell_fun_02.frame(df4,isellrange,rng)
    ...
    ddf6=iSell_fun_02.frame(ddf5,isellrange,rng)
...
def CM_eZeeNoCM(otasold,ratepl,pcdata,cap,ftr,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    logging.debug('Module:CMAs, SubModule:CM_eZee')
    ...
    index=pd.date_range(tday,periods=isellrange+rng)
    ...
    pc_ezee_frame = iSell_fun_02.frame(pc_ezee, isellrange,rng)
...
def CM_eZee(cmdata,ratepl,pcdata,ftr,isellrange, htlname,rng):
    logging.debug('------------------------------------------------------------')
    logging.debug('Module:CMAs, SubModule:CM_eZee')
...
def CM_UK(otadata,cmrate,msrate,isellrange,rng):
    logging.debug('------------------------------------------------------------')
...
def CM_Djubo(ttlsold,cap,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    ...
    ttlsold = iSell_fun_02.frame(ttlsold,isellrange,rng)
...
def CM_Maximojo(cmdata,msrate,rateid,ftr,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    ...
    df_avail = iSell_fun_02.frame(df_cm3,isellrange,rng)
    ...
    cmrate4 = iSell_fun_02.frame(cmrate3,isellrange,rng)
...
def CM_AxisRooms(cmdata,pcdata,ftr,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    ...
    cm_df4=iSell_fun_02.frame(cm_df3,isellrange,rng)
...
def CM_Eglobe(cmdata,pcdata, ftr,msrate, ratepl,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    ...
    msrateframe = iSell_fun_02.frame(cmfin111, isellrange,rng)
    ##-------------------------------------------------------------------------------------------------------------
...
def CM_AsiaTech1(cmdata,pcdata, ftr,msrate, ratepl,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    ...
    try:
        ...
        df_cmdata['Date'] = pd.to_datetime(df_cmdata['Date'])
...
#------------------------------------------------------------------------------------------
#------------------------------A.S. 29May23------------------------------------------
def CM_Q2B(cmdata,msrate,rateid,ftr,isellrange,rng):#pcdata
    logging.debug('------------------------------------------------------------')
    logging.debug('Module:CMAs, SubModule:CM_Maximojo')
...
#-------------------A.S. 17May2023----------------------------------------------------------
def CM_Cloudbeds(cmdata,pcdata, ftr,msrate, ratepl,isellrange,htlname,rng):
    logging.debug('------------------------------------------------------------')
    logging.debug('Module:CMAs, SubModule:CM_AsiaTech1')
 #-----------------------------------------------------------------------
...
#---------------------------------------------------------------------------------------
    ...
    in_data1 = iSell_fun_02.frame(in_data1, isellrange,rng) #A.S
    availframe = iSell_fun_02.frame(cmfin3, isellrange,rng)
...
def CM_Staah(cmdata,msrate,ftr,isellrange,rng):
    logging.debug('------------------------------------------------------------')
...
def CM_Staah_Max(cmdata, msrate, ftr, isellrange,rng):
    cm2 = pd.DataFrame(cmdata[cmdata.isnull().all(axis=1)])
    cm3 = list(cm2.index)
    cmdata1 = pd.DataFrame(cmdata.iloc[1:cm3[0], :])
    ...
    msrateframe = iSell_fun_02.frameMax(cmfin111, isellrange,rng)
    availshaped = iSell_fun_02.cmreshapeMax(cmdata1)
    reqcols = list(availshaped.columns[1:])
    availshaped['Rooms Avail To Sell Online'] = availshaped.loc[:, reqcols].sum(axis=1)
    cmfin3 = availshaped.loc[:, ['Date', 'Rooms Avail To Sell Online']]
    availframe = iSell_fun_02.frameMax(cmfin3, isellrange,rng)
    fthrou = availshaped.loc[:, ['Date', ftr]]  # flowthrough
    fthrou.columns = ['Date', ftr]
    availframe2 = pd.merge(availframe, fthrou, on='Date', how='left')
    return (availframe2, msrateframe)
...
def CM_TB_Normal(cmdata,msrate,rateid,ftr,isellrange,rng):
    logging.debug('------------------------------------------------------------')
    logging.debug('Module:CMAs, SubModule:CM_Maximojo')
...
def CM_StayFlexi(cmdata, msrate, ftr, isellrange,rng):
    logging.debug('------------------------------------------------------------')
...
def CM_Phobs(cmdata, isellrange,rng):
    logging.debug('------------------------------------------------------------')
...
def CM_Synxis(cmdata, msrate, ftr, isellrange,rng):
    logging.debug('------------------------------------------------------------')
...
def CM_Avails(cmdata,htlname,msrate,ftr,chman,pcdata,ratepl,isellrange,cmdata2,rng):
    logging.debug('------------------------------------------------------------')
    logging.debug('Module:CMAs, SubModule:CM_Avails')

    if chman == 'Staah':
        dfa,dfb = CM_Staah(cmdata,msrate,ftr,isellrange,rng)

    elif chman == 'StayFlexi':
        dfa, dfb = CM_StayFlexi(cmdata, msrate, ftr, isellrange,rng)

    elif chman == 'Synxis':
        dfa, dfb = CM_Synxis(cmdata, msrate, ftr, isellrange,rng)

    elif chman == 'Phobs':
        dfa, dfb = CM_Phobs(cmdata,isellrange,rng)

    elif chman == 'AsiaTech1':
        dfa, dfb = CM_AsiaTech1(cmdata,pcdata,ftr, msrate, ratepl, isellrange,rng)

    elif chman == 'AxisRooms':
        print("AxisRooms - CM Availability and Rate Fetch")
        dfa,dfb = CM_AxisRooms(cmdata,pcdata,ftr,isellrange,rng)

    elif chman == 'Maximojo':
        dfa,dfb = CM_Maximojo(cmdata,msrate,ratepl,ftr,isellrange,rng)

    elif chman == 'eZee':
        dfa,dfb = CM_eZee(cmdata,ratepl, pcdata,ftr,isellrange, htlname,rng)

    elif chman in ['TB','TB1']:
        dfa,dfb = CM_TB_Normal(cmdata,ratepl, pcdata,ftr,isellrange,rng)

    elif chman == 'ResAvenue':
        print("ResAvenue - CM Availability and Rate Fetch")
        dfa,dfb = CM_ResAvenue(cmdata,pcdata,ftr,isellrange,rng)

    elif chman == 'BookingHotel':
        print("BookingHotel - CM Availability and Rate Fetch")
        dfa,dfb = BookingHotel_CM(pcdata, cmdata, ftr, msrate, isellrange,rng)

    elif chman == 'TravelClick':
        print("TravelClick - CM Availability and Rate Fetch")
        dfa, dfb = TravelClick(pcdata, cmdata,cmdata2, ftr, msrate, ratepl, isellrange,rng)

    elif chman == 'Eglobe':
        dfa, dfb = CM_Eglobe(cmdata,pcdata,ftr, msrate, ratepl, isellrange,rng)

    elif chman == 'Staah Max':
        dfa, dfb = CM_Staah_Max(cmdata,ftr, msrate, isellrange,rng)

    elif chman == 'Q2B':     #A.S. 5Jun23
        dfa, dfb = CM_Q2B(cmdata,msrate,ratepl,ftr,isellrange,rng)#pcdata

    elif chman == 'Cloudbeds':     #A.S. 17May2023
        dfa, dfb = CM_Cloudbeds(cmdata, pcdata, ftr, msrate, ratepl, isellrange,htlname,rng)
